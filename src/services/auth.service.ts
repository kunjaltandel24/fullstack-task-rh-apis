...
export default class AuthService implements IAuthService {
	...
	async userVerification(bodyParams: IUserVerificationParams): Promise<{
		...
		const stripeCustomer = await this.stripeService.createCustomer(user.email, user.username, user._id.toString())
		const stripeAccount = await this.stripeService
			.createConnectedAccount(user.email, user.username, user._id.toString())
